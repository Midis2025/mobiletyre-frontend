...
import { Phone, MessageCircle, ArrowRight, ChevronDown, Menu, X } from 'lucide-react';
    ...
    <>
      <nav className="flex items-center justify-between px-4 md:px-10 h-20 md:h-28 bg-gray-100 shadow-sm relative z-50">
        ...
        <div className="flex items-center gap-3">
          <button className="hidden lg:flex items-center gap-2 bg-[#FB7E10] text-white px-5 py-2.5 rounded-full font-bold text-sm shadow-lg hover:bg-orange-600 transition-all whitespace-nowrap">
      ...
      {isMenuOpen && (
        <div className="lg:hidden bg-gray-100 fixed top-20 left-0 w-full shadow-lg border-t border-gray-200 z-40 py-4 px-6 flex flex-col space-y-4 font-semibold text-gray-800 shadow-b-2xl animate-in slide-in-from-top-2 duration-200">
          <a href="#" className="flex items-center w-full py-2 hover:text-[#FB7E10] border-b border-gray-200">Services</a>
          <a href="#" className="flex items-center w-full py-2 hover:text-[#FB7E10] border-b border-gray-200">Tires</a>
          <a href="#" className="flex items-center w-full py-2 hover:text-[#FB7E10] border-b border-gray-200">Locations</a>
          <a href="#" className="flex items-center w-full py-2 hover:text-[#FB7E10]">FAQ</a>
          
          <button className="flex items-center justify-center gap-2 bg-[#FB7E10] text-white w-full py-3.5 rounded-xl font-bold text-sm shadow-lg hover:bg-orange-600 transition-all mt-2">
            <span>CALL NOW: +0800 123 4567</span>
            <Phone size={16} fill="white" />
          </button>
        </div>
      )}
...
const Features = () => {
  return (
    <div className="bg-white py-12 md:py-20 px-4 sm:px-10 lg:px-20">
      {/* Logos Row */}
      <div className="flex flex-wrap justify-center md:justify-between items-center gap-6 md:gap-12 opacity-80 mb-12 md:mb-20 grayscale hover:grayscale-0 transition-all duration-500">
        {[1, 2, 3].map((i) => (
          <div key={i} className="flex flex-col leading-none scale-75 md:scale-100">
            <span className="font-black italic text-2xl md:text-3xl tracking-tighter text-black">MOBILE TYRE</span>
            <span className="font-extrabold text-[9px] md:text-[12px] tracking-[0.4em] self-end mt-[-3px] md:mt-[-5px] text-black">CREW</span>
          </div>
        ))}
      </div>
    ...
    <footer className="bg-[#0B1528] pt-16 md:pt-20 pb-8 md:pb-10 px-4 sm:px-10 lg:px-20 border-t border-white/5">
        ...
        <div className="lg:mt-2 sm:col-span-2 lg:col-span-1">
          <div className="bg-[#050B16] border border-white/5 rounded-2xl p-6 space-y-5 text-center sm:text-left">
            <div className="space-y-1">
              ...
              <h4 className="text-white font-black text-2xl lg:text-3xl italic tracking-tight">0800 123 4567</h4>
            </div>
            <button className="w-full bg-[#FB7E10] text-white py-3 md:py-3.5 rounded-xl font-black uppercase text-xs md:text-sm tracking-widest hover:bg-orange-600 transition-all active:scale-[0.98] shadow-lg shadow-orange-900/10">
              Call Now
            </button>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto pt-10 md:pt-20 mt-10 md:mt-20 border-t border-white/5 flex flex-col items-center justify-between gap-4">
        <div className="flex items-center gap-4 md:gap-6 order-1 md:order-2">
          <span className="text-gray-600 text-[10px] md:text-xs font-semibold cursor-pointer hover:text-white transition-colors">UK-WIDE SERVICE</span>
          <div className="w-1 h-1 md:w-1.5 md:h-1.5 rounded-full bg-[#FB7E10]"></div>
          <span className="text-gray-600 text-[10px] md:text-xs font-semibold cursor-pointer hover:text-white transition-colors">EST. 2012</span>
        </div>
        <p className="text-gray-500 text-[10px] md:text-xs font-medium order-2 md:order-1 text-center font-['Inter']">© 2026 Mobile Tyre Crew. All rights reserved.</p>
      </div>
    </footer>
  );
};
